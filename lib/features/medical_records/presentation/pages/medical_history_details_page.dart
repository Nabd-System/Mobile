...
class MedicalHistoryDetailsPage extends StatelessWidget {
  ...
  const MedicalHistoryDetailsPage({super.key, required this.historyId});
    ...
    return BlocProvider(
      create: (_) =>
          MedicalRecordsBloc.create()
            ..add(GetMedicalHistoryDetailsEvent(id: historyId)),
      child: Scaffold(
        appBar: AppBar(
          title: Text('Medical History', style: AppTextStyles.heading3()),
          centerTitle: true,
          leading: IconButton(
        ...
        body: BlocBuilder<MedicalRecordsBloc, MedicalRecordsState>(
          builder: (context, state) {
            // Loading
            if (state.medicalHistoryDetailsStatus == RequestStatus.loading) {
              return const Center(
            ...
            // Error
            if (state.medicalHistoryDetailsStatus == RequestStatus.error) {
              return Center(
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Text(state.medicalHistoryDetailsError),
                    const SizedBox(height: 16),
                    ElevatedButton(
                      onPressed: () {
                        context.read<MedicalRecordsBloc>().add(
                          GetMedicalHistoryDetailsEvent(id: historyId),
                        );
                      },
            ...
            // Success
            final details = state.medicalHistoryDetails;
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // ==================== Treatment ====================
                  if (details.treatment.isNotEmpty) ...[
                    _buildSectionTitle('TREATMENT'),
                    const SizedBox(height: 12),
                    _buildTreatmentCard(details.treatment),
                    const SizedBox(height: 20),
                  ],
  ...
  // ==================== Header Card ====================
  Widget _buildHeaderCard(MedicalHistoryDetailsModel details) {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [
            _getStatusColor(details.status),
            _getStatusColor(details.status).withValues(alpha: 0.7),
          ],
        ...
        boxShadow: [
          BoxShadow(
            color: _getStatusColor(details.status).withValues(alpha: 0.3),
            blurRadius: 12,
  ...
  // ==================== Info Card ====================
  Widget _buildInfoCard(MedicalHistoryDetailsModel details) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
  ...
  // ==================== Timeline Card ====================
  Widget _buildTimelineCard(MedicalHistoryDetailsModel details) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
